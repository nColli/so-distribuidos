...
int main(int argc, char *argv[]) {
    ...
    int interval = 3; // Update logical clock every 3 seconds

    fd_set readfds;
    while (1) {
        // Automatic logical clock tick every 3 seconds
        if (seconds_elapsed % interval == 0) {
        ...
        sleep(1); // Ensures the loop runs once per second
    }
    close(server_fd);
    return 0;
}
