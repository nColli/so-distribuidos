...
void handle_connections() {
    ...
    while (1) {
        ...
        printf("Redirigiendo a hilo\n");

        pthread_create(&thread_id, NULL, handle_client, (void*)&fd_socket_client);
...
void* handle_client(void* arg) {
    int *fd_socket_client = (int*)arg;
    printf("Thread recibio fd del cliente: %d\n", *fd_socket_client);
